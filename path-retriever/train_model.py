...
from dataset_load_dgl import load_data
...
class Trainer_KBQA(object):
    ...
    def save_ckpt(self, reason="h1"):
        ...
        model_name = os.path.join(self.args['checkpoint_dir'], "{}-{}_hw.ckpt".format(self.args['experiment_name'],
                                                                                   reason))
    ...
    def extract_topk_triples(self, data, k=5, split="test", save_path=None):
        """
        질문별로 상위 k개의 triple을 추출한다.
        - data: self.test_data 또는 self.valid_data
        - k: 추출할 triple 개수
        - split: 'test' or 'valid'
        - save_path: 저장 경로 (jsonl 형식)
        """
        self.model.eval()
        results = []
        with torch.no_grad():
            data.reset_batches(is_sequential=False)
            num_batch = int(np.ceil(data.num_data / self.test_batch_size))

            for i in range(num_batch):
                batch = data.get_batch(i, self.test_batch_size, fact_drop=0.0)

                # 모델 forward: 엔티티별 점수 분포를 얻음
                scores = self.model.predict_entity(batch)  
                # predict_entity는 entity score 반환하도록 모델에 추가해줘야 함
                # scores: [B, num_entity]

                q_entities = batch['q_entity']  # 질문별 시작 엔티티 id
                for b, qid in enumerate(batch['ids']):
                    q_ent = q_entities[b].item()
                    topk_val, topk_idx = torch.topk(scores[b], k)

                    triples = []
                    for rank, ent_id in enumerate(topk_idx.tolist()):
                        triples.append((self.id2entity[q_ent],
                                        "pred_rel",  # relation 정보가 필요하다면 edge lookup 추가
                                        self.id2entity[ent_id]))
