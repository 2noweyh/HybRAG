...
# CUDA_VISIBLE_DEVICES=0 python data_preprocess/dgl_extractor_v2.py --dataset cwq
...
def create_kgqa_dgl_graph(df, group_map_path, text_encoder, max_depth=1):
    ...
    #         weights_shards = []
    #         for i in tqdm(range(0, len(rel_texts), rel_batch), ...):
    #             batch = rel_texts[i:i+rel_batch]
    #             emb = text_model(batch)                 # [B, D]
    #             w = emb.norm(dim=1).to('cpu').to(torch.float16)
    #             weights_shards.append(w)
    #             del emb

    #         # etype별로 바로 cat → 저장 구조로 사용
    #         eweight = torch.cat(weights_shards, dim=0)
    #         eweight_dict[etype] = eweight
    #         del weights_shards, eweight
...
def main():
    ...
    # print(f"  - Edge weights: {save_dir}/eweight_dict.pt")
